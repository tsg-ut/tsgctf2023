...
// copy from how2heap decrypt_sefe_linking
unsigned long long safe_decrypt(unsigned long long cipher)
{
    unsigned long long key = 0;
    unsigned long long plain;

    // change 6 to 7 for decrypting 16 nibble fully
    for (int i = 1; i < 7; i++) {
        int bits = 64 - 12 * i;
        if (bits < 0) bits = 0;
...
int main()
{
    ...
    memset(key_st, '\0', sizeof(struct RC4_KEY));

    // rc4 key (extract from binary)
    ...
    for (int i = 0; i < 64;i++)
    {
        unsigned long long* t = (unsigned long long*) & chk[i * 8];
        *t = safe_decrypt(*t);
    };

    // decrypt rc4
    RC4(key_st, 512, chk, dec);

    for (int i = 0; i < 64;i++)
    {
        unsigned long long* t = (unsigned long long*) & dec[i * 8];
        *t = safe_decrypt(*t);
    };
